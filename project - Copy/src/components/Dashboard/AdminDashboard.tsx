...
import { Download, Users, AlertTriangle, Archive, TrendingUp } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const handleDownloadReport = (type: 'pdf' | 'csv') => {
    toast.success(`System-wide ${type.toUpperCase()} report downloaded successfully!`);
  ...
  const stats = [
    ...
    {
      name: 'Avg Confidence',
      value: `${(allClassifications.reduce((sum, c) => sum + c.confidence, 0) / allClassifications.length || 0).toFixed(1)}%`,
      icon: TrendingUp,
      color: 'bg-purple-500',
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Hazardous Elements and Trend */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Top Hazardous Elements */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Top Hazardous Elements</h3>
          {hazardousChartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={hazardousChartData} layout="horizontal">
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" />
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">All Classifications</h3>
